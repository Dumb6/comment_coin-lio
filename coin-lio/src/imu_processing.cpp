  ...
  std::vector<int>& vec_idx)
{
  ...
  std::unordered_map<int, std::vector<int>> timestamp_map;  // doc：根据每个点的时间戳存储到哈希表中
  timestamp_map.reserve(pcl_out.points.size());
  for (size_t idx = 0; idx < pcl_out.points.size(); idx++)
  {
    const int t_ns = pcl_out.points[idx].curvature * 1e6;   // doc：ms转ns
    timestamp_map[t_ns].push_back(idx);                     // doc：同一ns的点放在一起
  }

  /*** extract individual timestamps and sort times in ascending order ***/
  std::vector<int> timestamps;                              // doc：从小到大，对哈系表中每个时间戳进行排序
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    in.acc = acc_avr;
    in.gyro = angvel_avr;
    Q.block<3, 3>(0, 0).diagonal() = cov_gyr; //过程噪声协方差矩阵
  ...
  for (size_t idx = 0u; idx < timestamps.size(); idx++) {
    for (int p_idx : timestamp_map[timestamps[idx]]) {  //它会遍历timestamp_map[timestamps[idx]] 返回的向量中的每个元素
